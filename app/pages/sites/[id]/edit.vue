    ...
    <div class="w-full max-w-xl relative z-10">
      ...
      <UCard class="glass-card rounded-2xl border-neutral-200/50 dark:border-white/10 ring-0 p-6 shadow-2xl relative">
        ...
        <UForm :state="state" :schema="schema" @submit="onSubmit" class="flex flex-col gap-8">
          <div v-if="pageLoading" class="p-20 text-center">
          ...
          <div v-else class="flex flex-col gap-6">
            <UFormField :label="t('add_website.node_name')" name="name" class="premium-label" :help="t('add_website.node_name_help')">
              <UInput 
                ...
                size="xl"
                class="w-full"
                :ui="{ base: 'py-3 px-4 text-neutral-900 dark:text-white bg-transparent border-0 ring-0 hover:bg-transparent focus:ring-0 focus:bg-transparent' }"
            ...
            <UFormField :label="t('add_website.endpoint_url')" name="url" class="premium-label" :help="t('add_website.endpoint_url_help')">
              <UInput 
                ...
                class="w-full"
                :ui="{ base: 'py-3 ps-10! px-4 text-neutral-900 dark:text-white bg-transparent border-0 ring-0 hover:bg-transparent focus:ring-0 focus:bg-transparent' }"
...
import { navigateTo, useRoute, useAuth, useRuntimeConfig, useNuxtApp } from '#imports';
...
async function fetchData() {
  ...
  try {
    ...
    checkTypes.value = (typesResponse.data as CheckType[]).filter(c => c.is_active);
...
async function onSubmit() {
  ...
  try {
    const { $fetch } = useNuxtApp();
    const checks = Object.keys(state.value.selectedChecks)
      .map(id => ({ id: parseInt(id), ...state.value.selectedChecks[parseInt(id)] }))
      .filter(item => item.enabled)
      .map(item => ({
        check_type_id: item.id,
        params: item.params
      }));
